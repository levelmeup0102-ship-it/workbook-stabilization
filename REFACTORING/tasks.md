...
|------|------|------|
| 1 | `/auth` | pass (현재 스킵) |
| 2 | `/passage` (GET, POST) | 분석 완료, 작업 등록됨 |
| 3 | `/generate` (POST) | 분석 완료, 작업 등록됨 |
| 4 | DELETE 관련 (3개) | 분석 완료, 작업 등록됨 |
...
- [ ] **extract_base_data() (기존 step1) 제거**
  ...
  - vocab, test_a/b/c: Stage 1 전용 Claude 호출로 분리
...
- [ ] **검증/재시도 로직 공통화**
  ...
  - 로깅도 이 과정에서 통일 (현재 logger와 `_safe_print()` 혼용 중)

- [ ] **`_run_async()` 우회 패턴 제거 + pipeline async 전환**
...
- [ ] 프론트엔드 파일 분리 (HTML/CSS/JS 단일 파일 → css/, js/ 분리)
  - 프레임워크 도입 불필요 (페이지 1개, 상태 3개, 오버엔지니어링)
  - Vanilla JS 유지, 파일 분리만 진행: css/index.css, js/api.js, js/ui.js, js/auth.js, js/main.js
  - 프레임워크 필요 시점: 페이지 5개 이상 또는 복잡한 폼/상태 관리 발생 시
